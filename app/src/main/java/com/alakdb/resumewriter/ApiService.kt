...
suspend fun generateResumeFromFileToText(
    ...
    try {
        ...
        Log.d("ApiService", "📝 Job desc length: ${jobDescription.length} chars")
        Log.d("ApiService", "🎭 Tone: $tone")

        // Convert resume URI to file
        val resumeFile = uriToFile(resumeUri)
        Log.d("ApiService", "✅ Resume file selected: ${resumeFile.name}, size: ${resumeFile.length()} bytes")
        ...
        // ✅ CRITICAL FIX: Use the authenticated client (with SafeAuthInterceptor)
        val request = Request.Builder()
            .url("$baseUrl/generate-resume-from-file-to-text")
            .addHeader("Accept", "application/json")
            // ✅ The SafeAuthInterceptor will automatically add X-User-ID header
            .post(body)
            .build()

        Log.d("ApiService", "➡️ Sending FILE→TEXT resume generation request to: $baseUrl/generate-resume-from-file-to-text")

        // ✅ Use the authenticated client that includes the SafeAuthInterceptor
        client.newCall(request).execute().use { response ->
            val respBody = response.body?.string() ?: "{}"
            Log.d("ApiService", "📬 FILE→TEXT response: ${response.code}")
            Log.d("ApiService", "📬 Response body preview: ${respBody.take(200)}...")

            return@withContext if (response.isSuccessful) {
                try {
                    Log.d("ApiService", "✅ API call successful, parsing response...")
                    val jsonResponse = JSONObject(respBody)
                    val resumeResponse = GenerateResumeResponse(
                        success = jsonResponse.optBoolean("success", false),
                        resume_text = jsonResponse.optString("resume_text", ""),
                    ...
                    Log.d("ApiService", "✅ Response parsed - Success: ${resumeResponse.success}")
                    Log.d("ApiService", "✅ Resume text length: ${resumeResponse.resume_text.length}")
                    Log.d("ApiService", "✅ Remaining credits: ${resumeResponse.remaining_credits}")
                ...
                val errorMsg = "HTTP ${response.code}: ${response.message} – $respBody"
                Log.e("ApiService", "❌ API error: $errorMsg")
                ApiResult.Error(errorMsg, response.code)
            }
        }
    } catch (e: Exception) {
        Log.e("ApiService", "❌ Exception in generateResumeFromFileToText: ${e.message}", e)
    ...
    // Always use the preferred filename format
    val formattedFilename = if (filename.contains("generated_resume_")) {
        // Extract the generation ID if available
        val generationId = filename.substringAfter("generated_resume_").substringBeforeLast(".")
        val extension = filename.substringAfterLast(".")
        "SkillSync_Resume_${generationId}.${extension}"
    } else if (!filename.startsWith("SkillSync_Resume")) {
        val extension = filename.substringAfterLast(".")
        "SkillSync_Resume.${extension}"
    } else {
        filename
    }
    ...
    try {
        ...
        Log.e("FileSave", "❌ Failed to save file: ${e.message}", e)
        // Fallback to cache directory if Downloads fails
